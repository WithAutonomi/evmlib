...
fn get_evm_network_from_env() -> Result<Network, Error> {
    let evm_vars = [
        ...
        env::var(PAYMENT_VAULT_ADDRESS)
            .ok()
            .or_else(|| PAYMENT_VAULT_ADDRESS_BUILD_TIME_VAL.map(|s| s.to_string())),
    ]
    .into_iter()
    .map(|var| {
        var.ok_or(Error::FailedToGetEvmNetwork(format!(
            "missing env var, make sure to set all of: {RPC_URL}, {PAYMENT_TOKEN_ADDRESS}, {PAYMENT_VAULT_ADDRESS}"
        )))
    })
    ...
    if use_arbitrum_one {
        ...
        let network = CustomNetwork::new(&evm_vars[0], &evm_vars[1], &evm_vars[2]);
